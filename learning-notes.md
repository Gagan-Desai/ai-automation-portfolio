...
## Days 17-18 — Stress-Testing the Classifier, and the Real Limit of Self-Reported Confidence

**Day 17 goal:** deliberately break the classify-then-route architecture from Day 16, not the extraction layer already stress-tested on Day 15. Two adversarial tests, both exposing the same real architectural gap.

**The gap:** the original `DocumentType` enum was closed — `job_application`/`reference_letter`/`offer_acceptance`, no escape hatch. A closed-set classifier structurally cannot say "none of these fit"; it's forced to guess.

**Test 1 — a misfiled document from an unrelated domain** (`invoice_meridian.pdf` dropped into the HR folder): forced into `job_application`, extraction correctly failed after 3 retries since no invoice content could satisfy that schema's required fields. A safe failure, but only by luck of field-name mismatch, not by design.

**Test 2 — a document structurally similar to an existing type but semantically different** (a resignation letter, same letterhead/justified-body/signature-block shape as `ReferenceLetter`): **misclassified differently across separate runs** — `offer_acceptance` once, `job_application` once, `offer_acceptance` again — genuinely inconsistent, not a stable wrong answer, and never once used the `unknown` category even after it was explicitly added and the prompt explicitly permitted it. **Real, precise lesson: a prompt instruction shifts probability, it does not guarantee behavior.**

**A specific, real failure mode found via the full retry log, not just the final error:** attempting to force the resignation letter into `OfferAcceptance` caused the model to **hallucinate an entirely new field** (`end_date`) not present in the schema at all, populated with real data from the source (the resignation's actual effective date). The corrective retry loop partially responded across 3 attempts (changing other fields to `None`) while **never fully resolving the specific flagged error** — even setting `end_date: null` still violated `extra="forbid"`, since that config forbids the *key* being present at all, not just requiring a non-null value. **Concrete limit of corrective retry: feeding back a specific error does not guarantee the model fixes that specific thing.**

**The fix:** added `unknown` to the `DocumentType` enum, explicit prompt permission to abstain, and a new `needs_review` status (distinct from `failed`) in `process_folder.py` — the system correctly recognizing its own limits, not a crash.

**Day 18 — two confidence approaches, deliberately compared, not just one built on faith:**
- **Approach A (self-reported):** a separate `assess_confidence()` pass, per-field high/medium/low with reasoning, kept structurally separate from the extraction schemas themselves.
- **Approach B (self-consistency):** run identical extraction N times, compare field-by-field agreement via `.model_dump()` (Pydantic → plain dict, needed since raw Pydantic objects and Enum members aren't natively JSON-serializable).
- **Real, measured finding proving Approach A's known weakness:** the *same* document (a deliberately-built "hedge letter" — vague, non-committal reference language, a real HR phenomenon) produced **different confidence assessments across two separate runs of identical code** — one run correctly flagged two genuinely ambiguous fields as low-confidence with sound reasoning; another run on the same input flagged nothing. **Self-reported LLM confidence is not just occasionally wrong — it's measurably non-deterministic on the identical input.**
- **A related, real API-design bug caught along the way:** the initial API only surfaced `low_confidence_fields` (a filtered list of names), silently discarding the full reasoning and `medium` results — meaning genuine nuance was being thrown away before a human reviewer ever saw it. Fixed by exposing the complete `confidence_details` array in the API response.

## Day 19 — Structured Logging: Persisted, Structured, Queryable

**Goal:** replace every `print()` (gone the moment the terminal closes) with real logging — JSON Lines format (one complete JSON object per line, written via a single `json.dumps()` call per event), enabling later programmatic analysis rather than re-reading scrollback.
**Real gotcha, caught by the log system itself:** `logger` was referenced inside `generic_extractor.py`'s retry loop but never defined in that file's own scope — a genuine `NameError` that crashed mid-write, caught in the very first real log file generated. **Fixed by calling `setup_logger()` a second time from a different file** — safe specifically because `logging.getLogger(name)` is a global registry keyed by name, always returning the identical logger object regardless of which file calls it, and the `if not logger.handlers:` guard prevents a duplicate file handler from ever being attached.
**The real payoff, once fixed:** aggregated evidence across multiple real runs, not a single anecdote — confirmed via `analyze_log.py` parsing the JSON Lines file, that the resignation letter's classification leaned consistently toward `offer_acceptance`/`job_application` and never once resolved to `unknown` across every logged run — a genuinely citable, measured finding for the portfolio write-up.

## Day 20 — Wrapping Project 1 in a Real FastAPI Async Job API

**Goal, grounded in a real number from Day 19's own logs:** one document took ~12 seconds end-to-end. A synchronous REST endpoint blocking that long is bad design — the async job pattern (`BackgroundTasks`, immediate `202 Accepted` response with a job ID, `GET /status/{job_id}` polling) is the correct, standard fix.
**Two distinct Pydantic model layers, deliberately separated:** document *content* schemas (Day 15-16's `Invoice`/`JobApplication`) vs. the API's own *request/response contract* (`ExtractResponse`, `StatusResponse`) — genuinely different concerns kept apart.
**Real environment friction, same root cause recurring across three separate tools this week** (`python-dateutil` on Day 15, then `uvicorn`): `which python3` correctly pointed at the conda env, but `which uvicorn` resolved to system Python 3.14 instead — two different Python installations with separate package sets. **Fixed, and adopted as a permanent habit: always invoke pip-installed CLI tools via `python3 -m <tool>` rather than the bare command, removing PATH ambiguity entirely.**
**A genuinely new file-upload mechanism:** `UploadFile`/`File(...)` for real binary uploads via multipart/form-data (the first endpoint built accepting binary content rather than JSON), and `io.BytesIO(contents)` to let `pdfplumber` read in-memory bytes as if they were an open file, with no temp file written to disk.
**Honest, stated limitation:** the in-memory `jobs` dict has no persistence across restarts and no cross-process sharing — a deliberate, documented scope simplification, not an oversight.

## Day 21 — Production README, Portfolio Piece #1

Documented the problem (fixed RPA templates vs. classify-then-route), the architecture (with a live-rendering Mermaid diagram, confirmed to render correctly on GitHub natively, no separate image needed), every real design decision with its actual reasoning, and — the section doing the most real credibility work — a **"Real findings from testing"** section stating every genuine failure discovered this week precisely: the unreliable `unknown` classification rate, the hallucinated `end_date` field, the non-deterministic confidence assessment, and the watermark-corrupted-text resilience finding from Day 15. Extended later with a documented `/extract-batch` endpoint (one independent background job per file, deliberately not accepting a server-side folder path — a real path-traversal security consideration, stated explicitly rather than silently avoided).

## Day 22 — Real Integration Proof: Live API Stress Test + n8n Cross-System Call

**Part 1:** confirmed the same `unknown`/`needs_review` behavior already proven via the standalone script now holds through the actual live HTTP API layer too — a genuinely different code path, not previously tested.

**Part 2 — calling the API from n8n, the real "integrated into a larger system" proof:**
- **Real Docker networking gotcha, directly reusing Day 9's core lesson:** n8n runs in its own container; `localhost` there refers to the container itself, not the Mac running the FastAPI server. Fixed using Docker Desktop's special DNS name, `host.docker.internal`, resolving from any container back to the host machine — same underlying concept as `postgres-fx` container-to-container resolution, just solving the reverse direction.
- **Diagnosed a "your request is invalid" API error precisely, not by guessing:** confirmed connectivity was completely fine first (via `docker exec -it n8n node -e "require('http').get(...)"`, since the container's minimal image had no `curl` installed) — isolating the real problem to the HTTP Request node's own body configuration, not networking. The actual bug: **Body Content Type was set to "n8n Binary File"** (sends raw bytes with no field wrapping at all) instead of **"Form-Data"** with an explicit `file`-named binary field — FastAPI's `UploadFile` parser correctly rejected the mismatched format.
- **Real, unresolved platform limitation, revisited and finally worked around:** the `N8N_RESTRICT_FILE_ACCESS_TO` file-path bug from Project A recurred identically on a fresh container rebuild, even with the semicolon-separator variant tried. Rather than keep chasing a likely genuine upstream bug, switched the trigger from Read/Write Files from Disk to a **Form Trigger with a file-upload field** — arguably a more realistic integration test anyway (an external submission arriving into the workflow), and one that sidesteps the filesystem-permission issue entirely rather than depending on it being fixed.
- **Final, complete proof:** a real job ID returned from n8n's HTTP Request node, confirming a fully separate containerized system successfully called the custom-built API as a black box and received a correct, structured async response.

---

## Day 23 — RAG Fundamentals: What It Is, Types, Embeddings, Chunking

**The real problem RAG solves:** an LLM's knowledge is frozen at training time, with zero access to private/current data — and critically, it doesn't reliably say "I don't know" when asked about something outside its knowledge, it generates plausible-sounding text anyway. **RAG's actual insight:** retrieve real, relevant source material *before* generating, then have the model synthesize an answer from that evidence rather than pure memory — trading "recall from memory" for "reason over retrieved evidence," which is inherently more traceable and verifiable.

**The core loop:** embed the question (same model as the documents — two different embedding models produce mathematically incompatible vector spaces, even for identical text) → search the vector store for closest matches → augment the prompt with the retrieved chunks → generate. This is naive/simple RAG — the actual foundation being built this week.

**Real landscape beyond naive RAG, worth knowing exists:** hybrid search (combining vector similarity with keyword/BM25 search, since pure embeddings can miss exact matches on names/numbers/legal wording); re-ranking (retrieve more candidates than needed, then re-rank with a slower, more precise model); query rewriting; multi-hop/agentic RAG (a direct preview of Week 6-7's agent work); GraphRAG (relationship-structured retrieval instead of flat chunks).

**Embeddings, precisely:** dense vectors (384 dimensions for `all-MiniLM-L6-v2`) produced by an encoder model trained via contrastive learning — related text pairs pulled together, unrelated pairs pushed apart, during training. **Cosine similarity is the industry-standard distance metric for text specifically because it's magnitude-invariant** — a long verbose chunk and a short terse one expressing the same idea shouldn't score as "different" just because one vector is numerically larger; only direction (meaning) matters. Real production gotcha: some embedding model families are asymmetric, requiring different prefixes for queries vs. documents — `all-MiniLM-L6-v2` is symmetric and doesn't need this. Real operational constraint: swapping embedding models makes every existing stored vector meaningless — no migration path, full re-embedding required.

**Chunking, precisely:** the "lost in the middle" effect (LLMs demonstrably pay less attention to information buried mid-context, even when it technically fits) means retrieval-then-generation with focused chunks often beats stuffing a whole long document in, even when the whole thing would fit. Four real strategies: fixed-size, sentence-aware/recursive, semantic (group consecutive sentences only while their embeddings stay similar, splitting at real topic shifts), and document-structure-aware. **Parent-child (hierarchical) chunking** is a genuinely valuable production pattern worth remembering: search against small precise chunks, but return the larger parent section to the LLM — decoupling retrieval precision from generation context instead of one chunk size forced to serve both.

## Day 24 — Real Finance-Industry Corpus: 8 Live Regulatory Documents, Two Chunking Strategies Compared

**Deliberate scope change from the original plan:** rather than reusing Project 1's synthetic HR/invoice documents, sourced 8 genuinely real, current, publicly available PDFs directly from the Central Bank of Ireland's own domain — all part of the March 2026 Consumer Protection Code overhaul. Real, meaningful thematic variety: general consumer-protection guidance (4 docs), the regulatory consultation process itself (2 docs, genuinely different writing style — discursive/Q&A vs. directive), and insurance-sector-specific requirements (2 docs) — deliberately distinct enough that a good retrieval system needs to actually discriminate between them, not just match everything to everything.

**Real engineering practice, not just downloading files:** a `requests`-based download script with a `User-Agent` header (some servers reject non-browser-looking requests) and a `time.sleep(1)` between calls — considerate rate-limiting against a real government server, a genuinely good habit for any scraping/ingestion pipeline.

**Extraction defensive fix, specific to real-world PDFs this domain surfaced for the first time:** `if page.extract_text()` guards against pages that are pure images (scanned signatures, graphic cover pages) — `extract_text()` returns `None` for such pages, and joining a list containing `None` crashes outright; this filter skips them silently.

**The overlap concept, proven and fully understood on real data, not just described:** fixed-size chunking still cuts individual chunks at arbitrary points — overlap does **not** prevent any single chunk from looking broken at its edges. What it actually guarantees is different and more specific: whatever got cut off at one chunk's end reappears **complete** inside the *next* chunk, since that chunk starts further back. Verified directly on real numbered regulatory clauses (`"...customers' interests. 2."` cut off in one chunk, the full context re-appearing intact at the start of the next) — real, reproducible evidence of the mechanism working, not a theoretical claim. **Real, honest limit worth remembering:** this only holds if the content needing to stay together is shorter than the overlap window itself — content spanning further than that would still fracture across both neighbors.

**Real, measured proof the embedding pipeline works before trusting it further:** `util.cos_sim` on two genuinely paraphrased sentences sharing zero overlapping words ("strong analytical skills" vs. "exceptional analytical rigor") scored `0.656`; an unrelated pair (analytical skills vs. an invoice total) scored `-0.052`, correctly near zero rather than strongly negative (unrelated topics aren't opposites, so near-zero is the mathematically correct result, not a weak one). **The ~0.71 gap between them is the concrete, self-generated evidence the model is capturing real semantic meaning**, not a diagram taken on faith.

**Deliberate architectural choice for Day 25's actual comparison:** stored both chunking strategies in **separate ChromaDB collections** (`documents_fixed`, `documents_sentence`) rather than one merged collection — enables querying both with identical real questions later and directly comparing retrieval quality, rather than mixing strategies into one pool where results couldn't be cleanly attributed. Real, measured result from storing both: sentence-aware produced 1,658 chunks vs. fixed-size's 1,594 (a ~4% increase) — the quantified cost of never fracturing a sentence mid-thought, refusing to top up a chunk to full capacity if the next whole sentence wouldn't fit.
